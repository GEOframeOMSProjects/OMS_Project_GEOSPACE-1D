...
def write_grid_netCDF_richards_solute_ade(eta, eta_dual, z, z_dual, space_delta, control_volume, control_volume_index, psi_0, T_0, C_0, equation_state_ID, parameter_ID, KMAX, theta_s, theta_r, par_1, par_2, par_3, par_4, par_5, alpha_ss, beta_ss, ks,molecularDiffusion,longitudinalDispersivity, output_file_name, output_title, output_institution, output_summary, output_date, grid_input_file_name, parameter_input_file_name):
    ...
    :param C_0: solute concentration initial condition.
    :type ic: array
    ...
    data_C_0 = ncfile.createVariable('C0','f8',('z'))
    data_C_0.units = 'M/m3'
    data_C_0.long_name = 'Solute concentration initial condition'
    ...
    data_molecularDiffusion = ncfile.createVariable('molecularDiffusion','f8',('parameter'))
    data_molecularDiffusion.units = 'm2/s'
    data_molecularDiffusion.long_name = 'molecular diffusion coefficient'
    
    data_longitudinalDispersivity = ncfile.createVariable('longitudinalDispersivity','f8',('parameter'))
    data_longitudinalDispersivity.units = 'm'
    data_longitudinalDispersivity.long_name = 'longitudinal dispersivity'
    ...
    for i in range(0,dim_parameter):
        ...
        data_beta_ss[i] = beta_ss[i]
        data_ks[i] = ks[i]
        data_molecularDiffusion[i] = molecularDiffusion[i]
        data_longitudinalDispersivity[i] = longitudinalDispersivity[i]
